...
def _analyze_pylabrobot_error(simulation_output: str) -> dict:
    ...
    error_extraction_patterns = {
        ...
        "DeckConflictError": [
            r"already occupied",
            r"location conflict",
            r"cannot assign .* to deck",
            r"Deck.*conflict"
        ],
        "BackendNotReady": [
            r"backend not setup",
            r"backend.*not.*initialized",
            r"LiquidHandler.*not.*setup",
            r"call .*setup\(\) first"
        ],
        "TimeoutError": [
            r"timeout",
...
def prepare_feedback_node(state: PyLabRobotGraphState) -> PyLabRobotGraphState:
    ...
    if is_stuck:
        ...
        elif error_type == "DeckConflictError":
            analysis = "A deck/location conflict occurred while assigning resources to the deck."
            action = "Action: Verify two resources are not placed on the same deck location. Adjust positions in the hardware configuration so every resource has a unique location."

        elif error_type == "BackendNotReady":
            analysis = "The LiquidHandler backend is not initialized (setup not called) or failed to initialize."
            action = "Action: Ensure `await lh.setup()` is called before running operations. If this happens during environment setup, check backend selection in hardware config (backend field) and try the fallback (ChatterBoxBackend)."

        elif error_type == "TimeoutError":
            analysis = "A timeout occurred during simulation."
            action = "Action: Simplify the protocol, reduce loops or delays, and ensure no blocking operations are present."

        elif error_type == "TypeError":
            analysis = "A Python TypeError occurred, likely due to incorrect argument types or arity in an API call."
            action = "Action: Check the function/method signatures for the call site. Ensure volumes are numeric lists when required and resources are well proxies (e.g., tip_rack['A1'])."

        elif error_type == "ValueError":
            analysis = "A Python ValueError occurred, often indicating an invalid parameter (e.g., negative volume, invalid index)."
            action = "Action: Verify all parameter values are valid. Volumes must be positive and within instrument range; well names must match A1–H12."

        elif error_type == "KeyError":
            missing_key = entities.get("undefined_name", "unknown")
            analysis = f"A KeyError occurred, possibly referencing a missing mapping or dictionary key: {missing_key}"
            action = "Action: Ensure dictionary accesses are valid; check resource alias mapping and variable definitions."
...
async def run_pylabrobot_agent_and_stream_events(
    ...
    try:
        # Prepare accumulated latest state for final emission
        ...
        async for event in app.astream(
            ...
            # Accumulate latest state from node outputs for final result emission
        ...
        # After the stream is finished, build final result from the accumulated latest_state

        simulation_result = latest_state.get('simulation_result', {}) or {}
        success = bool(simulation_result.get('success', False))
        total_attempts = int(latest_state.get('attempts', 0))
        final_outcome = latest_state.get('final_outcome')
        generated_code = latest_state.get('python_code')
        ...
        yield {
            ...
            "message": f"PyLabRobot Agent completed after {total_attempts} attempts"
        }
